...
int dados = B1;
int desligar = B1;
int switchh=0, switch1=B000000,switch2=B000000;
int cmd,bobina;
int liga_antiga = 0;
...
void desligar_todas_bobinas(){

for(int sw=1;sw<=2;sw=sw+1){
  for(int var=1;var<=6;var=var+1){
    escolher_bobina(var);
      ...
      if(sw==1){ 
        passar_valores_b(dados);
        passar_valores_c(B111111);
      }else{
        passar_valores_b(B111111);
        passar_valores_c(dados);
      }
        delay(200); 
  }
  delay(1000);
...
void passar_valores_c(int valor){
    digitalWrite(A5, valor & (1 << 5)  ? 1 : 0);
    digitalWrite(A4, valor & (1 << 4)  ? 1 : 0);
    digitalWrite(A3, valor & (1 << 3)  ? 1 : 0);
    digitalWrite(A2, valor & (1 << 2)  ? 1 : 0);
    digitalWrite(A1, valor & (1 << 1)  ? 1 : 0);
    digitalWrite(A0, valor & (1 << 0)  ? 1 : 0);
